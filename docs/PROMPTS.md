...
If a custom `directory` is provided, Sashiko automatically resolves files using a "Convention over Configuration" approach. You do not need a `stages.toml` if your files follow the standard naming patterns.

### Implicit Stage Overrides
The registry looks for files in a `stages/` sub-directory matching the pattern `{number:02}-*.md`.
- `stages/01-goal.md` automatically replaces the hardcoded Stage 1 instruction.
- `stages/05-concurrency.md` automatically replaces the hardcoded Stage 5 instruction.

### Automatic Context Injection
Files in specific sub-directories are automatically gathered and injected into the AI's shared knowledge base:
- `subsystem/*.md`: Selected during "Phase 0" based on patch relevance.
- `patterns/*.md`: Always included as global technical guidelines.

### Standard Supporting Files
Specific stages look for these filenames in your custom directory by default:
- **Stage 3**: `callstack.md`, `technical-patterns.md`
- **Stage 5**: `subsystem/locking.md`
- **Stage 8**: `false-positive-guide.md`, `severity.md`
- **Stage 9**: `inline-template.md`
